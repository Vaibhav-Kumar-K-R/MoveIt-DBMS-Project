...
import { useEditEmployeeRequest } from "@/api/ManagersApi";
...
const SummarySection = ({ employeeData, employee_id }: SummarySectionProps) => {
  const { controls } = useMultiStepFormContext();
  const { editEmployee, isLoading } = useEditEmployeeRequest(employee_id);
